...
H_TEXTURE(g_CustomMotionVectors);
...
{
    return LOAD_TEXTURE2D_X(g_HTraceGBuffer0, pixCoord);
}

float2 GetMotionVector(uint2 pixCoord)
{
    float2 MotionVectors;
    DecodeMotionVector(LOAD_TEXTURE2D_X(g_CustomMotionVectors, pixCoord), MotionVectors);
