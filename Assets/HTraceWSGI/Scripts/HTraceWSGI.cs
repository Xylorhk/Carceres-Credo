...
namespace HTraceWSGI.Scripts
{
	...
	public class HTraceWSGI : MonoBehaviour, IPing
	{
		...
		public ReflectionIndirectLightingData ReflectionIndirectLightingData = new ReflectionIndirectLightingData();
		...
		private void OnEnable()
		{
			...
			HResources.ReflectionIndirectLightingData = ReflectionIndirectLightingData;
		...
		private void InitComponents()
		{
			...
			_prePassObject   = _passService.GetOrCreateCustomPassObject<PassHandler>(CustomPassInjectionPoint.AfterOpaqueDepthAndNormal, 
				passName: HTraceNames.HTRACE_PRE_PASS_NAME,   priority: 10,  layer: gameObject.layer, typeof(HTracePrePass));
			_mainPassObject  = _passService.GetOrCreateCustomPassObject<PassHandler>(CustomPassInjectionPoint.AfterOpaqueDepthAndNormal, 
				passName: HTraceNames.HTRACE_MAIN_PASS_NAME,  priority: -20, layer: gameObject.layer,  typeof(HTraceMainPass));
			_finalPassObject = _passService.GetOrCreateCustomPassObject<PassHandler>(CustomPassInjectionPoint.BeforePostProcess,         
				passName: HTraceNames.HTRACE_FINAL_PASS_NAME, priority: -30, layer: gameObject.layer,        typeof(HTraceFinalPass));
			_voxelObject = _passService.GetOrCreateCustomPassObject<VoxelsHandler>(CustomPassInjectionPoint.AfterOpaqueDepthAndNormal, 
