	...
	internal class VoxelOctantCamera : MonoBehaviour
	{
		...
		private void MoveAxisX(float offset)
		{
			...
			switch (VoxelizationRuntimeData.OctantIndex)
			{
				case OctantIndex.OctantA:
					finalPos.x += -offset * HResources.VoxelizationData.ExactData.Bounds.x / 4;
		...
		private void MoveAxisY(float offset)
		{
			...
			switch (VoxelizationRuntimeData.OctantIndex)
			{
				case OctantIndex.OctantA:
					...
					finalPos.y += -HResources.VoxelizationData.ExactData.Bounds.y / 4;
					finalPos.z += offset * HResources.VoxelizationData.ExactData.Bounds.z / 4;
					break;
				case OctantIndex.OctantB:
					finalPos.x += HResources.VoxelizationData.ExactData.Bounds.x / 4;
					finalPos.y += HResources.VoxelizationData.ExactData.Bounds.y / 4;
		...
		private void MoveAxisZ(float offset)
		{
			...
			switch (VoxelizationRuntimeData.OctantIndex)
			{
				case OctantIndex.OctantA:
					finalPos.x += -HResources.VoxelizationData.ExactData.Bounds.x / 4;
					finalPos.y += offset * HResources.VoxelizationData.ExactData.Bounds.y / 4;
					finalPos.z += HResources.VoxelizationData.ExactData.Bounds.z / 4;
					break;
				case OctantIndex.OctantB:
					finalPos.x += HResources.VoxelizationData.ExactData.Bounds.x / 4;
		...
		private void SetParams(OctantIndex octantIndex)
		{
			...
			_camera.farClipPlane     = HResources.VoxelizationData.ExactData.Bounds.z / (2 * scale);
			_camera.nearClipPlane    = -HResources.VoxelizationData.ExactData.Bounds.z / (2 * scale);
			_camera.orthographicSize = .5f * HResources.VoxelizationData.ExactData.Bounds.x / scale;
			_camera.aspect           = 1;
		}
