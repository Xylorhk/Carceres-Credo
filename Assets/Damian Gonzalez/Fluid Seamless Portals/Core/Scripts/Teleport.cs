    ...
    public class Teleport : MonoBehaviour {
        ...
        Vector3 GetVelocity(Transform obj) {
            ...
            if (obj == setup.player.playerMainObj) {
                ...
                if (setup.player.controllerType == PortalSetup.Player.ControllerType.Rigidbody) return setup.player.playerRb.linearVelocity;
            } else {
                if (obj.TryGetComponent(out Rigidbody rb)) return rb.linearVelocity;
        ...
        void DoTeleport(Transform objectToTeleport, bool fireEvents = true) { //note: we're still inside OnTrigger...
            ...
            if (rb != null) {
                rb.linearVelocity = otherScript.portal.TransformDirection(
                    portal.InverseTransformDirection(rb.linearVelocity)
                );
